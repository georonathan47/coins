...
abstract class PaymentRemoteDatabase {
  ...
  Future<String> activateAccount(UserPaymentDetail account, Map tokens);
}
...
class PaymentRemoteDatabaseImpl implements PaymentRemoteDatabase {
    ...
    try {
      ...
      if (result.statusCode! >= 200 && result.statusCode! < 300) {
        TLoggerHelper.logApiResult(
          ...
          message:
              'Deleted pay account with id $id for user ${tokens['userId']}',
        );
        return;
      } else if (result.statusCode! == 403) {
        TLoggerHelper.logRefreshAttempt(
  ...
  @override
  Future<String> activateAccount(UserPaymentDetail request, Map tokens) async {
    try {
      final result = await client.put(
        '${Env.activateAccountUrl}${request.id}',
        headers: {'Authorization': 'Bearer ${tokens['accessToken']}'},
        body: jsonEncode({
      ...
      TLoggerHelper.logEvent(
        'Activate account with id ${request.id} for user ${tokens['userId']}, response: ${result.bodyString}, status code: ${result.statusCode}',
      );
      if (result.statusCode! >= 200 && result.statusCode! < 300) {
        TLoggerHelper.logApiResult(
          httpMethod: 'PUT',
          ...
          message: 'Activated pay account for user ${tokens['userId']}',
        );
        return jsonDecode(result.bodyString!)['response'];
      } else if (result.statusCode! == 403) {
        TLoggerHelper.logRefreshAttempt(
          'activateAccount',
          statusCode: result.statusCode!,
        ...
        try {
          ...
          tokens['accessToken'] = token.accessToken;
          tokens['refreshToken'] = token.refreshToken;
          return activateAccount(request, tokens);
        } catch (e) {
          throw ServerException(result.statusText!);
        }
      } else {
      ...
      TLoggerHelper.logError(
        method: 'activateAccount',
        error: e,
        eventName: 'Activating account for user ${tokens['userId']}',
        message: e.toString(),
      );
      throw DeviceException('Unexpected Error!\nPlease try again later');
    }
